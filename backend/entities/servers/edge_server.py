...
# Now you can import the module as if it were directly available
from app import PPO_MODEL  # Adjust the import path as necessary
...
HARDWARE_VEHICLE = ["INTEL_I7", "NANO"]
HARDWARE_EDGE = ["INTEL_I7", "NANO","TX2"]
HARDWARE_CLOUD = ["INTEL_I7", "NANO", "TX2","AGX"]
HARDWARE_NAMES = ["INTEL_I7", "NANO", "TX2", "AGX"]
...
class EdgeServer(BaseServer):
    ...
    def __init__(
        ...
        power_P_e_c: float,
        network:Network,
        vehicles_servers_list: Optional[list["VehicleServer"]],
        # tracker :Tracker,
        env: Environment,
    ):
        super().__init__(
        ...
        self.vehicles_servers_list =vehicles_servers_list
    ...
    def receive_tasks(self, task_model_platform: TaskModelPlatform):
        # Track task receiving
        if self.tracker:
            self.tracker.track_task_receiving(task_model_platform.task_id, self.id, self.env.now)

        self.add_task_to_queue(task_model_platform)
        print(f"TIME {self.env.now:.3f}: [INFO] Task {task_model_platform.task_id} received by Edge {self.name}")
    ...
    def add_task_to_queue(self, task: TaskModelPlatform):
        ...
        # Track task added to queue
        if self.tracker:
            self.tracker.track_task_added_to_queue(
                task.task_id, self.id, task.position_in_edge_queue, self.env.now
            )

        print(f"TIME {self.env.now:.3f}: [INFO] Task {task.task_id} added to {self.name} queue")
    ...
    def _extract_tasks_from_store(self, max_tasks: int):
        ...
        for _ in range(max_tasks):
            ...
            # self.tracker.track_task_event(task, "extracted_from_queue", {"extraction_time": task.extraction_time_to_edge_queue})
            task.extraction_time_to_edge_queue = self.env.now
            # Track task extraction
            if self.tracker:
                self.tracker.track_task_extraction(task.task_id, self.id, self.env.now)
    ...
    def batch_extractor_agent(self):
        ...
        """
        print(f"TIME {self.env.now:.3f}: [AGENT] Batch extractor agent started for {self.name}.")
        while True:
            try:
                ...
                # self.tracker.track_server_action(self, "batch_extracted", {"batch_size": len(batch), "task_ids": [t.task_id for t in batch if t]})
            ...
            for model in self.deployed_models:  # number of models per type is at max 3
                if model.type == _type:
                    for platform in self.processing_platforms_list:  # number of platforms is 2
                        ...
                        # self.tracker.track_task_event(task, "executions_appended", {"edge_executions": len(task.executions_dict['E'][1]), "cloud_executions": len(task.executions_dict['C'][1])})
    ...
    def calculate_task_cost_outputes(
        ...
        print(
            f"TIME {self.env.now:.3f}:  Starting cost calculation and offloading decisions for {len(generated_task_data_list)} tasks"
        )
        ...
        for tasks_batch in generated_task_data_list:
            ...
            edge_execution_details = []
            self.cost_model.estimate(tasks_batch,"E")
            # self.cost_model.estimate(tasks_batch)
            for _, execution in tasks_batch.executions_dict['E'][1].items():
                edge_execution_details.append({
            ...
            if self.tracker:
                self.tracker.track_cost_estimation(Level.EDGE, tasks_batch.task_id, self.env.now, edge_execution_details)

            if self.cloud_server and self.cloud_server.cost_model:
                cloud_execution_details = []
                self.cloud_server.cost_model.estimate(tasks_batch,"C")
                # self.cloud_server.cost_model.estimate(tasks_batch)
                for _, execution in tasks_batch.executions_dict['C'][1].items():
                    cloud_execution_details.append({
                        "model_name": execution.model.name if execution.model else "unknown",
                        "platform_name": execution.platform.name if execution.platform else "unknown",
                        "execution_time": execution.execution_time,
                        "memory_consumption": execution.memory_consumption,
                ...
                if self.tracker:
                    self.tracker.track_cost_estimation(Level.CLOUD, tasks_batch.task_id, self.env.now, cloud_execution_details)

        print(f"TIME {self.env.now:.3f}:  Cost estimation completed for all tasks")
        ...
        # print("...........................................")
        # print(len(rl_inputs))
        # print(len(generated_task_data_list))
        input_preparation_time = time.time() - start_time
        # Track offloading input preparation
        if self.tracker:
            self.tracker.track_offloading_input_preparation(
                self.id, self.env.now, len(generated_task_data_list), input_preparation_time
            )

        print(f"TIME {self.env.now:.3f}:  RL inputs prepared in {input_preparation_time:.4f}s")
        ...
        print(f"TIME {self.env.now:.3f}:  Starting offloading decision calculation...")
        ...
        print("//////////////////////////////////////")
        if rl_inputs is None:
            print("[ERROR] rl_inputs is None! Task batch:", [t.task_id if t else None for t in generated_task_data_list])
        else:
            print(f"[DEBUG] rl_inputs keys: {list(rl_inputs.keys())}")  # Safe summary        print(tasks_with_decisions)
        # Track offloading decision calculation
        if self.tracker:
            self.tracker.track_offloading_decision_calculation(
                self.id, self.env.now, len(generated_task_data_list), decision_time, "PPO",tasks_with_decisions
            )
        print(f"TIME {self.env.now:.3f}:  Offloading decisions completed in {decision_time:.4f}s")
        # 4. Send results back to vehicles
        yield from self._send_decisions_to_vehicles(tasks_with_decisions)

        print(f"TIME {self.env.now:.3f}:  All tasks processed and sent back to vehicles")
        ...
        for _, level_execution in task_model_platform.executions_dict.items():
            for __, execution in level_execution[1].items():
                ...
                for i, waypoint in enumerate(task_model_platform.vehicle.route_plan[:-1]):
                    ...
                    print(
                        f"TIME {self.env.now:.3f}:  Vehicle {task_model_platform.vehicle.name} reached destination at {task_model_platform.vehicle.position}"
                    )
                    return
                ...
                # print(
                #     f"TIME {self.env.now:.3f}:  Vehicle {task_model_platform.vehicle.name} after {execution.duration_for_future_position} will be at position {task_model_platform.vehicle.position}"
                # )
    ...
    def prepare_offloading_inputs(self, task_batch: list[Optional[TaskModelPlatform]]) -> dict:
        ...
        """
        # if not task_batch or all(t is None for t in task_batch):
        #     print("[WARNING] Empty task batch in RL inputs prep")
        #     return {}  # Or default dict
        for task in task_batch:
            print(f"Time {self.env.now:.3f}: Type of task generates by the vehicle {task.vehicle.id} : {task.task.type}")
        # Initialize the input dictionary
        inputs = {
            "Tasks_Data": {
                "id_Tache": np.full((N_MAX, 1), -1, dtype=np.int64),
                "Type": np.full((N_MAX, 1), -1, dtype=np.int32),
            ...
            "Vehicles_Data": {
                ...
                "hardware_name_id": np.full(
                    (V_MAX, len(HARDWARE_VEHICLE), 1), -1, dtype=np.int64
                ),
            ...
            "Info_Edge": {
                ...
                "hardware_name_id": np.full(
                    (len(HARDWARE_EDGE), 1), -1, dtype=np.int64
                ),
                "charge_remaining_percentage": np.full(
            ...
            "Info_Cloud": {
                ...
                "hardware_name_id": np.full(
                    (len(HARDWARE_CLOUD), 1), -1, dtype=np.int64
                ),
                "charge_remaining_percentage": np.full(
        ...
        for i, task_model_platform in enumerate(task_batch[:N_MAX]):  # Limit to N_max
            ...
            inputs["Tasks_Data"]["Type"][i, 0] = (  # type: ignore
                TASK_TYPES.index(task.type) if task.type in TASK_TYPES else -1
            )
        ...
        for i, vehicle in enumerate(self.vehicles_servers_list):
            ...
            for j, platform in enumerate(
                ...
                charge_remaining = platform.platform_usage.level
        ...
        for task_type in TASK_TYPES:
            for i,default_execution in enumerate(default_executions[task_type]):
                model_id = (
                    MODEL_NAMES.index(default_execution["model_name_id"])
                    if default_execution["model_name_id"] in MODEL_NAMES
                    else -1
                )
                hw_id = (
                    HARDWARE_NAMES.index(default_execution["hardware_name_id"])
                    if default_execution["hardware_name_id"] in HARDWARE_NAMES
                    else -1
                )
                niveau_id = LEVELS.index(default_execution["niveau_id"]) if default_execution["niveau_id"] in ["V", "E", "C"] else -1
                accuracy = self.normalize_value(default_execution["accuracy"],"min_accuracy")
                execution_time = self.normalize_value(default_execution["execution_time"],"execution_time")
                power_consumption = self.normalize_value(default_execution["power_consumption"],"power_consumption")
                memory_consumption = self.normalize_value(default_execution["memory_consumption"],"memory_consumption")
                utilization_percentage = self.normalize_value(default_execution["utilization_percentage"],"utilization_percentage")
                # Fill combination data with normalization
                inputs["Combinations_Data"][task_type]["model_name_id"][i, 0] = model_id  # type: ignore
                inputs["Combinations_Data"][task_type]["hardware_name_id"][i, 0] = hw_id  # type: ignore
                inputs["Combinations_Data"][task_type]["niveau_id"][i, 0] = niveau_id  # type: ignore
                inputs["Combinations_Data"][task_type]["accuracy"][i, 0] = accuracy # type: ignore
                inputs["Combinations_Data"][task_type]["execution_time"][i, 0] = execution_time # type: ignore
                inputs["Combinations_Data"][task_type]["power_consumption"][i, 0] = power_consumption # type: ignore
                inputs["Combinations_Data"][task_type]["memory_consumption"][i, 0] =memory_consumption # type: ignore
                inputs["Combinations_Data"][task_type]["utilization_percentage"][ i, 0] = utilization_percentage # type: ignore
        ...
        #             )
        #             hw_id = (
        #                 HARDWARE_NAMES.index(execution.platform.name)
        ...
        if not inputs:  # Safety check
            print("[ERROR] Empty inputs dict built!")
            return {}
        return inputs
    ...
    def _calculate_offloading_decisions_process(
        ...
        print(f"[DEBUG0] {len(task_batch)}")
        if algorithme == "RL":
            try:
                print(
                    f"TIME {self.env.now:.3f}:  Initializing PPO model for offloading decisions..."
                )
                ...
                print(
                    f"TIME {self.env.now:.3f}:  PPO algorithm executed in {algorithm_execution_time:.4f}s"
                )
                ...
                for task in tasks_with_decisions:
                    if task and task.chosen_execution:
                        ...
                        print(
                            f"TIME {self.env.now:.3f}:  Task {task.task_id} → {'CLOUD SERVER' if task.chosen_execution.level==2 else 'EDGE SERVER' if task.chosen_execution.level==1 else 'VEHICLE'} on {task.chosen_execution.platform.name}"
                        )
                    elif task:
                        print(f"[WARNING] Task {task.task_id} has no chosen execution!")

                print(
                    f"TIME {self.env.now:.3f}:  Valid decisions: {valid_decisions_count}/{len([t for t in tasks_with_decisions if t])}"
                )
                ...
                print(f"TIME {self.env.now:.3f}:  Falling back to default decisions...")
                ...
                yield self.env.timeout(0.01)  # Small delay for fallback processing
                return tasks_with_decisions
        else:
            fallback_algo = OffloaingAlgorithme()
            tasks_with_decisions = fallback_algo._fallback_decisions(task_batch)
    ...
    def _send_decisions_to_vehicles(self, tasks_with_decisions: list[Optional[TaskModelPlatform]]):
        ...
        """
        print(f"TIME {self.env.now:.3f}:  Sending offloading decisions to vehicles...")

        # Group tasks by vehicle for efficient communication
        vehicle_task_groups = self._group_tasks_by_vehicle(tasks_with_decisions)
        
        # Track sending offloading decisions
        if self.tracker:
            vehicle_decisions = {vehicle_id: len(tasks) for vehicle_id, tasks in vehicle_task_groups.items()}
            self.tracker.track_sending_offloading_decisions(self.id, self.env.now, vehicle_decisions)
        ...
        if vehicle_processes:
            ...
            print(f"TIME {self.env.now:.3f}:  All offloading decisions sent to vehicles")
        else:
            print(f"TIME {self.env.now:.3f}:  No valid vehicle communications to send")
    ...
    def _group_tasks_by_vehicle(
        ...
        print(
            f"TIME {self.env.now:.3f}:  Grouped {len([t for t in tasks_with_decisions if t])} tasks into {len(vehicle_groups)} vehicle groups"
        )
    ...
    def _send_to_specific_vehicle(self, vehicle_id: int, vehicle_tasks: list[TaskModelPlatform]):
        ...
        try:
            ...
            print(
                f"TIME {self.env.now:.3f}:  Sending {len(vehicle_tasks)} decisions to Vehicle {vehicle_id}"
            )

            # Calculate communication data size (decision metadata)
            decision_data_size = len(vehicle_tasks) * 0.16  # Assume 16 Kilo bytes per decision

            # Simulate network transmission time
            yield from self.network.transmit(
                    decision_data_size,self,target_vehicle
                )

            print(
                f"TIME {self.env.now:.3f}:  Transmitting {decision_data_size} bytes to Vehicle {vehicle_id})"
            )

            # Call vehicle's offloading function
            print(f"TIME {self.env.now:.3f}:  Triggering task execution on Vehicle {vehicle_id}")
            ...
            print(f"TIME {self.env.now:.3f}:  Successfully sent decisions to Vehicle {vehicle_id}")
